...
pub struct META_EXTTEXTOUT {
    ...
    pub fw_opts: std::collections::BTreeSet<crate::ExtTextOutOptions>,
    ...
    pub rectangle: Option<crate::Rect>,
...
impl META_EXTTEXTOUT {
    ...
    pub fn parse<R: std::io::Read>(
        ...
        record_function: u16,
        charset: crate::CharacterSet,
    ) -> Result<Self, crate::ParseError> {
        use unicode_segmentation::UnicodeSegmentation;

        crate::records::check_lower_byte_matches(
            record_function,
            crate::RecordType::META_EXTTEXTOUT,
        )?;
        ...
        let fw_opts = {
            let (value, c) = crate::read_u16_from_le_bytes(buf)?;
            record_size.consume(c);

            let mut fw_opts = std::collections::BTreeSet::new();

            for v in [
                crate::ExtTextOutOptions::ETO_OPAQUE,
                crate::ExtTextOutOptions::ETO_CLIPPED,
                crate::ExtTextOutOptions::ETO_GLYPH_INDEX,
                crate::ExtTextOutOptions::ETO_RTLREADING,
                crate::ExtTextOutOptions::ETO_NUMERICSLOCAL,
                crate::ExtTextOutOptions::ETO_NUMERICSLATIN,
                crate::ExtTextOutOptions::ETO_PDY,
            ] {
        ...
        let rectangle = if fw_opts
            .contains(&crate::ExtTextOutOptions::ETO_OPAQUE)
            || fw_opts.contains(&crate::ExtTextOutOptions::ETO_CLIPPED)
        {
            let (v, c) = crate::Rect::parse(buf)?;
            record_size.consume(c);
        ...
        let string = {
            let (bytes, c) = crate::read_variable(buf, string_length as usize)?;
            record_size.consume(c);
        ...
        if record_size.remaining() {
            for _ in 0..string.graphemes(true).count() {
                let (v, c) = crate::read_i16_from_le_bytes(buf)?;
